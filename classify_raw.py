...
def prepare_data(signals_unseen, signals_seen):
...
def prepare_data_shuffle(signals_unseen, signals_seen, path):
    ...
    for i in range(len(list_seen) - 1, len(list_seen) - number_seen - 1, -1):
        list_seen.pop(i)
    for i in range(
            len(list_unseen) - 1,
    ...
    f = open(path + "train.pkl", "wb")
    pickle.dump(train, f)
    f.close()
    f = open(path + "test.pkl", "wb")
    pickle.dump(test, f)
    f.close()
    f = open(path + "targets.pkl", "wb")
    pickle.dump(targets, f)
    f.close()
    f = open(path + "test_targets.pkl", "wb")
...
def train_per_channel(channel: int,
    ...
    with open(path_channel + 'raw/files/accuracies.txt',
              'w+') as acc_file, open(path_channel + 'raw/files/losses.txt',
                                      'w+') as loss_file:
    ...
    torch.save(predictions, path_channel + 'raw/files/preds.pt')
    torch.save(ground_truths, path_channel + 'raw/files/gts.pt')
